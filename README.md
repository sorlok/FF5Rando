...
* I'm calling this out in advance: most of my event edits don't call "RemoveTelepoCache", so you'd be left with a TelepoCacheItem on the stack. To trigger this bug, I'd expect you can go through a World 2 cutscene (beat Hiryuu Plant), then teleport to World 1, then walk into any Town and Teleport out (brings you to World 2). This could be fixed by modifying the "TeleportToWorldX" items to clear the Teleport stack, but there might be cleaner options available (and ideally we'd clean the stack ourselves in our modified Events).
* I think I broke "Vanilla Game" mode; it just spins waiting on something. Need to check the "stall" functions I put in place...
* More...
